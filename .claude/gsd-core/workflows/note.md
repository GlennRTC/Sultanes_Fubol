...
- **Global scope**: `D:/sultanes_fubol/Sultanes_Fubol/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/sultanes_fubol/Sultanes_Fubol/.claude/notes/`)
...
2. Glob `D:/sultanes_fubol/Sultanes_Fubol/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/sultanes_fubol/Sultanes_Fubol/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/sultanes_fubol/Sultanes_Fubol/.claude/notes/` — works in any directory
